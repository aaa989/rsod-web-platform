...
验证提交：刷新 GitHub 仓库页面，确认所有代码已成功上传，且**没有出现模型文件、数据集、node\_modules 等大文件**
...
注意：这里前端运行能成功的前提，你得是后端启动8000端口的同时，前端启动才能成功。一般就一种方法：

第一种：终端在后端和前端各开一个，一个只执行后端8000端口，一个在前端执行run.
...
1. **核心页面代码（`src/views/Inference.vue`）**
