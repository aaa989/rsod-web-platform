...
这里在测试的时候，先要把frontend/src/router/index.js中的

import Detection from '../views/index.vue'
改成
...
（windows输入）cat ~/.ssh/id_ed25519.pub
```
...
cd D:/app/基本组件/Desktop/rsod-web-platform
...
git remote set-url origin git@github.com:你的用户名/rsod-web-platform.git
...
注意：为什么不用`uvicorn main:app --port 8000`，而用`uvicorn main:app --host 0.0.0.0 --port 8000`？
答案：在这一步这两种方法都可以。0.0.0.0：允许所有人访问（包括前端）
但在接下来会前端测试后端，要求前端能够访问后端的`localhost:8000`端口，而我们应该开放端口，如果执行了`uvicorn main:app --port 8000`，会导致后边测试时，前端访问8000端口时被拒绝，使得测试失败。
另外，在7.5中更新了代码，涵盖了--host 0.0.0.0 --port 8000

访问`http://localhost:8000/docs`，看到 Swagger 接口文档即为成功。

注意：看清楚，一定是`http://localhost:8000/docs`，不是`http://localhost:8000`
...
const testConnect = async () => {
  try {
    const response = await fetch("http://localhost:8000/api/test/connect");
...
注意：在此之前需要，保持后端backend的uvicorn main:app --port 8000执行的同时执行以上代码才行。
还需要在在index.js中修改，
...
4. **完整测试流程**
   ...
   **步骤 3：测试接口连通性**（看这里）
   ...
   - 然后再执行刚才的3. **完整路由配置（`src/router/index.js`）**
