...
注意1：如果你之前安装过docker-Desktop,我说的是之前，不是指你刚才按照上边的方法刚安装的那些人，我建议你先完全地去更新wsl和docker-Desktop,再执行下边的程序，不然你会找问题找到破防。
如果有其他问题，看这个帖子，基本上都可以解决：`blog：https://blog.csdn.net/L2463323447/article/details/155704552`

注意2：如果你以前安装过虚拟机（比如说Vmware），你需要用 Docker Desktop 的 Hyper-V 模式，先执行以下步骤：

步骤1：恢复 Hyper-V 启动模式，确保 Hyper-V 已经启动。

```bash
# 管理员 PowerShell 执行
docker-compose up -dbcdedit /set hypervisorlaunchtype auto
```

步骤2：重启系统，执行以下操作：

开启 Hyper-V 功能：打开「控制面板 → 程序 → 启用或关闭 Windows 功能」
勾选：
...
步骤3：Docker Desktop 并切换到 Hyper-V 模式（部分版本里不需要设置）：

安装时，取消勾选 "Use the WSL 2 based engine"，选择使用 Hyper-V 引擎，然后启动 Docker，它会自动创建 Hyper-V 虚拟机，启动完成后左下角会变绿。

理由：VMware 和 Docker 的冲突根源：
当你用 VMware 时，Hyper-V 会被 VMware 禁用，或者反过来 Hyper-V 禁用 VMware。
你之前执行的 bcdedit /set hypervisorlaunchtype off 就是为了让 VMware 能用，结果导致 Docker/WSL2 依赖的虚拟化支持出了问题。
现在的死循环：
任务管理器显示虚拟化已启用，但 WSL2/Docker 还是报 “未启用虚拟化”，这是因为 Hyper-V/WSL2 的内核组件被 VMware 的设置干扰了。
...
```bash
md backend\app\api backend\app\models backend\app\services
md backend\app\utils backend\models backend\data

cd.>backend\app\__init__.py
cd.>backend\app\api\__init__.py
cd.>backend\app\models\__init__.py
cd.>backend\app\services\__init__.py
cd.>backend\app\utils\__init__.py
```
